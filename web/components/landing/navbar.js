...
const NAV_ITEMS = [
  ...
  { href: '/#modulos', label: 'Módulos', type: 'hash' },
  { href: '/#noticias', label: 'Noticias', type: 'hash' },
  { href: '/#personal', label: 'Personal', type: 'hash' },
  { href: '/#departamentos', label: 'Departamentos', type: 'hash' },
    ...
    <header id="landing-navbar" class="fixed inset-x-0 top-0 z-50 transition duration-200">
      <div class="landing-container flex h-16 items-center justify-between md:h-[4.5rem]">
        <a data-link href="/" class="group flex items-center gap-3">
          <span class="flex h-9 w-9 items-center justify-center rounded-2xl border border-brand-400/30 bg-brand-500/10 text-brand-300 transition duration-200 group-hover:bg-brand-500/20">
            ${icon('cross', 'h-5 w-5')}
          </span>
        ...
        <div class="hidden items-center gap-3 xl:flex">
          <a data-link href="/auth/register" class="text-sm font-medium text-ink-300 transition hover:text-white">Crear cuenta</a>
          <a data-link href="/auth/login" class="btn-primary !px-5 !py-2.5">Entrar</a>
        </div>
      ...
      <div id="landing-mobile-menu" class="hidden border-t border-white/10 bg-surface-950/95 backdrop-blur-xl xl:hidden">
        <div class="landing-container flex flex-col gap-1 py-4">
          ${NAV_ITEMS.map((item) => navAnchor(item, 'nav-link rounded-xl px-3 py-3 hover:bg-white/5')).join('')}
...
export function initNavbar(root = document) {
  ...
  const onScroll = () => {
    if (!navbar) return;
    if (window.scrollY > 24) navbar.classList.add('nav-glass');
    else navbar.classList.remove('nav-glass');
  };
  ...
  const onHashClick = (event) => {
    ...
    if (target) {
      ...
      if (mobileMenu && !mobileMenu.classList.contains('hidden')) onToggle();
    }
  };
