...
export function renderNavbar({ title = 'Dashboard' } = {}) {
  ...
  return `
    <header class="sticky top-0 z-30 border-b border-white/10 bg-surface-950/80 backdrop-blur-xl">
      <div class="flex h-16 items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <div class="flex min-w-0 items-center gap-3">
          <button
            type="button"
            data-open-sidebar
            class="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-ink-200 transition hover:bg-white/[0.08] hover:text-white lg:hidden"
            aria-label="Abrir menú"
          >
            ${icon('menu', 'h-4 w-4')}
          </button>
          <div class="min-w-0">
            <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-ink-500">Panel</p>
        ...
        <div class="flex items-center gap-2 sm:gap-3">
          ...
          <div class="relative" id="profile-menu">
            <button
              ...
              class="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] py-1.5 pl-1.5 pr-3 transition hover:border-white/20 hover:bg-white/[0.06]"
              aria-expanded="false"
              aria-haspopup="menu"
            >
              <span class="flex h-8 w-8 items-center justify-center overflow-hidden rounded-lg border border-white/10 bg-brand-500/15 text-xs font-semibold text-brand-200">
                ${
            ...
            <div id="profile-menu-panel" class="absolute right-0 z-50 mt-2 hidden w-56 overflow-hidden rounded-2xl border border-white/10 bg-surface-900/95 shadow-2xl backdrop-blur-xl">
              <div class="border-b border-white/10 px-4 py-3">
                <p class="truncate text-sm font-semibold text-white">${displayName}</p>
                <p class="truncate text-xs text-ink-400">${user?.username ?? user?.displayName ?? ''}</p>
              </div>
              <div class="p-2">
                <a data-link href="/profile" class="block rounded-xl px-3 py-2 text-sm text-ink-300 transition hover:bg-white/[0.04] hover:text-white">
                  Mi perfil
                </a>
                <a data-link href="/settings" class="block rounded-xl px-3 py-2 text-sm text-ink-300 transition hover:bg-white/[0.04] hover:text-white">
                  Configuración
                </a>
...
export function initNavbar(root = document) {
  ...
  const onDocumentClick = (event) => {
    if (!root.querySelector('#profile-menu')?.contains(event.target)) {
      close();
    }
  };

  const onLogout = () => logoutAndRedirect();
